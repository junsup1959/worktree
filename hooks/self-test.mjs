...
const HOOK_COMMAND = 'node "${PLUGIN_ROOT}/hooks/capture-learning-turn.mjs"';
...
for (const eventName of ["UserPromptSubmit", "Stop", "SessionEnd"]) {
  ...
  assert.equal(handlers[0].command, HOOK_COMMAND, eventName);
  assert.equal("commandWindows" in handlers[0], false, eventName);
  assert.doesNotMatch(
    handlers[0].command,
    /\b(?:powershell|pwsh)(?:\.exe)?\b|EncodedCommand/iu,
    eventName,
  );
...
try {
  ...
  const commandSmokeResult = await invoke(commandSmokeEvent);
  assert.equal(commandSmokeResult.stdout.trim(), "{}");
  assert.equal(commandSmokeResult.stderr, "");
